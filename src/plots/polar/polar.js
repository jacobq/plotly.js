...
proto.updateMainDrag = function(fullLayout, polarLayout) {
    ...
    function pathCornerForPolygons(r, va0, va1) {
        ...
        // TODO make these rectangle, no trapezoids
        // find perpendicular direction and ...

        return 'M' + ra2xy(rr - chw, a - da) +
            'L' + ra2xy(rr - chw, a + da) +
            'L' + ra2xy(rr + chw, a + da) +
            'L' + ra2xy(rr + chw, a - da) + 'Z';
    }
    ...
    function findPolygonRadius(x, y, va0, va1) {
        ...
        var dsin = Math.sin(va1) - Math.sin(va0);
        var dcos = Math.cos(va1) - Math.cos(va0);

        // find the intersection of the
        // va0 <-> va1 edge and (0,0) <-> va0 'radial' line
        if(dsin && dcos) {
            var m = dsin / dcos;
            var b = yy - m * xx;
            var mr = Math.sin(va0) / Math.cos(va0);
            var xstar = b / (mr - m);
            var ystar = mr * xstar;
            return norm(xstar, ystar);
        } else if(dcos) {
            return yy / Math.sin(va0);
        } else if(dsin) {
            return xx / Math.cos(va0);
        }
    }

    function _findPolygonRadius(x, y, va0, va1) {
        return norm(findIntersectionXY(va0, va1, va0, [x - cxx, cyy - y]));
    }
...
function findIntersectionXY(v0, v1, s, r) {
    ...
    // TODO handle cases when dsin=0, dcos=0
    var x = bv / (as - av);
    var y = as * x;
    return [x, y];
}
...
function _findIntersectionXY(v0, v1, a, xpyp) {
    var xstar, ystar;
...
function makeClippedPolygon(r, sector, vangles) {
    ...
    var xy0 = findIntersectionXY(vangles[i0], vangles[cycleIndex(i0 - 1)], s0, r);
    vertices.push(xy0);
    ...
    var xyN = findIntersectionXY(vangles[iN], vangles[cycleIndex(iN + 1)], s1, r);
    vertices.push(xyN);
